/**
 * Copied from netflix/concurrency-limits 0.1.7 and modified; original copyright notice below.
 *
 * See {@link https://github.com/Netflix/concurrency-limits/blob/a4c78dabf0e8d64cbb9ca4566f4ea1d3c9346543/concurrency-limits-core/src/main/java/com/netflix/concurrency/limits/limiter/BlockingLimiter.java}
 *
...
public final class BlockingLimiter<ContextT> implements Limiter<ContextT> {
     ...
     *
     * @param delegate Non-blocking limiter to wrap
     * @param timeout Max amount of time to wait for the wait for the limit to be released.
     *  Cannot exceed {@link BlockingLimiter#MAX_TIMEOUT}
     * @return Wrapped limiter
     */
    public static <ContextT> BlockingLimiter<ContextT> wrap(Limiter<ContextT> delegate, Duration timeout) {
        Preconditions.checkArgument(
                timeout.compareTo(MAX_TIMEOUT) < 0, "Timeout cannot be greater than " + MAX_TIMEOUT);
    ...
    public Optional<Listener> acquire(ContextT context) {
        return tryAcquire(context).map(acquiredListener -> new Listener() {
            @Override
            public void onSuccess() {
                acquiredListener.onSuccess();
                unblock();
            }

            @Override
            public void onIgnore() {
                acquiredListener.onIgnore();
                unblock();
            }

            @Override
            public void onDropped() {
                acquiredListener.onDropped();
                unblock();
            }
        });
    }
