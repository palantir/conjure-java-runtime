...
public final class RetrofitClientFactory {
    ...
    private static OkHttpClient newHttpClient(Optional<SSLSocketFactory> sslSocketFactory, OkHttpClientOptions options,
            String userAgent, Optional<ProxyConfiguration> proxyConfiguration) {
    ...
    public static <T> T createProxy(Optional<SSLSocketFactory> sslSocketFactoryOptional, String uri, Class<T> type,
        ...
        okhttp3.OkHttpClient client = newHttpClient(
                sslSocketFactoryOptional, options, userAgent, Optional.<ProxyConfiguration>absent());
        return createProxy(uri, type, client);
    }

    public static <T> T createProxy(Optional<SSLSocketFactory> sslSocketFactoryOptional, String uri, Class<T> type,
            OkHttpClientOptions options, String userAgent, ProxyConfiguration proxyConfiguration) {
        okhttp3.OkHttpClient client = newHttpClient(sslSocketFactoryOptional, options, userAgent,
                Optional.of(proxyConfiguration));
        return createProxy(uri, type, client);
    }

    private static <T> T createProxy(String uri, Class<T> type, OkHttpClient client) {
