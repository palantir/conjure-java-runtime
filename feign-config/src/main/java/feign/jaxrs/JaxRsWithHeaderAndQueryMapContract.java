 ...
 */
public final class JaxRsWithHeaderAndQueryMapContract extends Contract.BaseContract {
    ...
    protected MethodMetadata parseAndValidateMetadata(Class<?> targetType, Method method) {
        ...
        // perform extra validation for HeaderMap and QueryMap annotations
        Class<?>[] parameterTypes = method.getParameterTypes();
        if (data.headerMapIndex() != null) {
            feign.Util.checkState(Map.class.isAssignableFrom(parameterTypes[data.headerMapIndex()]),
    ...
    protected boolean processAnnotationsOnParameter(MethodMetadata data, Annotation[] annotations, int paramIndex) {
        ...
        // if any of the annotations on the parameter is a HeaderMap or QueryMap parameter, interpret it
        for (Annotation parameterAnnotation : annotations) {
            Class<? extends Annotation> annotationType = parameterAnnotation.annotationType();
            if (annotationType == HeaderMap.class) {
                feign.Util.checkState(data.headerMapIndex() == null,
